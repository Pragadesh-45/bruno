...
import {
  exportGlobalEnvironmentAsJson,
  exportCollectionEnvironmentAsJson
...
const ExportModal = ({ onClose, collection }) => {
  ...
  const selectedFormat = 'json'; // Only JSON format supported
    ...
    try {
      ...
      if (selectedGlobal.length > 0) {
        ...
        for (const env of selectedGlobal) {
          await exportGlobalEnvironmentAsJson(env, globalEnvPath);
        }
      }
      ...
      if (selectedCollection.length > 0) {
        ...
        for (const env of selectedCollection) {
          await exportCollectionEnvironmentAsJson(env, brunoEnvPath);
        }
      }
                      ...
                      <div className="environment-list">
                        {collection.environments.map((env) => (
                          <label key={env.uid} className="environment-item">
                            ...
                            <ToolHint
                              text={env.name}
                              toolhintId={`collection-env-${env.uid}`}
                              place="top"
                            >
                              <span className="environment-name">{truncateEnvName(env.name)}</span>
                            </ToolHint>
