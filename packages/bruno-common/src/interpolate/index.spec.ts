...
const calculateBrunoAge = (): number => {
  const today = new Date();
  let age = today.getFullYear() - BRUNO_BIRTH_DATE.getFullYear();
  const monthDiff = today.getMonth() - BRUNO_BIRTH_DATE.getMonth();

  if (monthDiff < 0 || (monthDiff === 0 && today.getDate() < BRUNO_BIRTH_DATE.getDate())) {
    age--;
  }
...
describe('interpolate', () => {
  it('should replace placeholders with values from the object', () => {
    ...
    const inputObject = {
      'user.name': 'Bruno',
      user: {
        age: calculateBrunoAge()
      }
    };

    const result = interpolate(inputString, inputObject);

    expect(result).toBe(`Hello, my name is Bruno and I am ${calculateBrunoAge()} years old`);
  });
...
`;
    const expectedStr = `
  Hi, I am Bruno,
  I am ${calculateBrunoAge()} years old.
...
`;
  ...
  it('should strictly match the keys (whitespace matters)', () => {
    ...
    expect(result).toBe(`Hello, my name is {{ user.name }} and I am ${calculateBrunoAge()} years old`);
  });
  ...
  test('should give precedence to the last key in case of duplicates (not at the top level)', () => {
    ...
    expect(result).toBe(`Hello, my name is Bruno and Not _Bruno_ I am ${calculateBrunoAge()} years old`);
  });
});
...
describe('interpolate - recursive', () => {
  ...
  it('should replace placeholders with 2 level of recursion with values from the object', () => {
    ...
    expect(result).toBe(`Hello, my name is Bruno Dog and I am ${calculateBrunoAge()} years old`);
  });
...
describe('interpolate - object handling', () => {
  it('should stringify simple objects', () => {
    ...
    const inputObject = {
      'user': { name: 'Bruno', age: calculateBrunoAge() }
    };
  ...
  it('should stringify simple objects (dot notation)', () => {
    ...
    const inputObject = {
      'user.data': { name: 'Bruno', age: calculateBrunoAge() }
    };
